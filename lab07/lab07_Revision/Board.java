...
public class Board {
    ...
    void placeDisposalItems() {
        int disposalsPlaced = 0;

        while (disposalsPlaced < MinesweeperGame.TOTAL_MINES / 2) {
            int row = random.nextInt(MinesweeperGame.ROWS);
            int col = random.nextInt(MinesweeperGame.COLS);

            if (!grid[row][col].isDisposalItem() && grid[row][col].isMine()) {
                grid[row][col].setDisposalItem(true);
                System.out.println(row + " " + col);
                disposalsPlaced++;
            }
        }
    ...
    boolean revealCell(int row, int col) {
        ...
        if (cell.isMine() && !cell.isDisposalItem()) {
            cell.reveal();
            return false;
        }
